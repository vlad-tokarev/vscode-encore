 ...
 * relative to an Encore app root (so the path points inside the app module).
 */
function packagePathFromFile(fileUri: vscode.Uri, appRootPath: string): string {
  const dir = path.dirname(fileUri.fsPath);
  const rel = path.relative(appRootPath, dir);
  return `./${rel.replace(/\\/g, "/")}`;
}

/**
 * Group test items by their package directory so each `encore test`
 * invocation covers one package. Each group is scoped to a specific
 * Encore app root, so cwd resolves to the right app when multiple
 * encore.app files live in the same workspace.
 */
interface PackageGroup {
  /** Absolute Encore app root path (used as cwd for `encore test`). */
  appRootPath: string;
...
async function groupByPackage(
  ...
  for (const item of items) {
    ...
    const appRootPath = await encoreAppState.getDiscoveredAppRootForFile(item.uri.fsPath);
    if (!appRootPath) {
      continue;
    }

    const pkgPath = packagePathFromFile(item.uri, appRootPath);
    const groupKey = `${appRootPath}::${pkgPath}`;

    let group = groups.get(groupKey);
    if (!group) {
      group = { appRootPath, packagePath: pkgPath, testNames: [], items: [] };
...
export async function runEncoreTests(
  ...
  const groups = await groupByPackage(itemsToRun, encoreAppState);
  ...
  for (const group of groups) {
    for (const item of group.items) {
      registerItemRecursive(item, group.appRootPath, itemLookup);
    }
  }
  ...
  for (const group of groups) {
    ...
    await runPackageTests(group, run, itemLookup, token);
  }
...
function registerItemRecursive(
  ...
  item.children.forEach((child) => {
    registerItemRecursive(child, appRootPath, itemLookup);
  });
}
...
export async function debugEncoreTest(
  ...
  if (!appRootPath) {
    vscode.window.showWarningMessage(
      "Encore: selected test does not belong to any discovered Encore app.",
    );
    return notStarted;
  }

  const workspaceFolder =
    vscode.workspace.getWorkspaceFolder(vscode.Uri.file(appRootPath))
  ...
  const relPkg = `./${path.relative(appRootPath, pkgDir).replace(/\\/g, "/")}`;
  ...
  const started = await vscode.window.withProgress(
    ...
    async (progress, progressToken): Promise<boolean> => {
      ...
      const compiled = await compileTestBinary(relPkg, binaryPath, appRootPath, combinedToken);
      if (!compiled) {
