...
import { ENCORE_APP_EXCLUDE_GLOB, GO_FILE_EXCLUDE_GLOB } from "../utils/workspaceScan";
  ...
  implements vscode.TreeDataProvider<EncoreTreeItem>
{
  ...
  private discoveredApps: DiscoveredEncoreApp[] = [];
  ...
  constructor(
    ...
    this.appRunner.onDidChangeState(() => {
      this.onDidChangeTreeDataEmitter.fire();
    });
    this.appRunner.onDidUpdateInfo(() => {
      this.onDidChangeTreeDataEmitter.fire();
    });
  ...
  async refreshProjectState(): Promise<void> {
    const encoreAppFiles = await vscode.workspace.findFiles(
      "**/encore.app",
      ENCORE_APP_EXCLUDE_GLOB,
    );

    const nextDiscoveredApps = encoreAppFiles
      .map((encoreAppFile) => buildDiscoveredEncoreApp(encoreAppFile.fsPath))
      .sort((left, right) =>
        left.displayName.localeCompare(right.displayName)
        || left.rootPath.localeCompare(right.rootPath),
      );

    this.secretStore.setDiscoveredAppRoots(
      nextDiscoveredApps.map((app) => app.rootPath),
    );

    if (!discoveredAppsEqual(this.discoveredApps, nextDiscoveredApps)) {
      this.discoveredApps = nextDiscoveredApps;
      this.folderTrees.clear();
      this.onDidChangeTreeDataEmitter.fire();
    }
  }
  ...
  async getChildren(element?: EncoreTreeItem): Promise<EncoreTreeItem[]> {
    if (!element) {
      ...
      rootItems.push(
        ...this.discoveredApps.map((app) => this.buildAppNode(app)),
      );
      return rootItems;
    }
    ...
    if (element.nodeKind === "appRoot") {
      ...
      if (this.secretStore.isAuthenticated()) {
        const app = this.discoveredApps.find((candidate) => candidate.rootPath === appRootPath);
        if (app) {
    ...
    if (element.nodeKind === "appRunRoot") {
      return this.buildAppRunChildren();
    }
    ...
    if (element.nodeKind === "databasesGroup") {
      ...
      return databases.map((db) => this.buildDatabaseNode(db));
    }
  ...
  private buildAppRunNode(): EncoreTreeItem {
    const appState = this.appRunner.getState();
    const appInfo = this.appRunner.getInfo();
  ...
  private buildAppRunChildren(): EncoreTreeItem[] {
  ...
  private buildAppNode(app: DiscoveredEncoreApp): EncoreTreeItem {
  ...
  private buildSecretsGroup(app: DiscoveredEncoreApp): EncoreTreeItem {
  ...
  private buildDatabaseNode(database: DatabaseInfo): EncoreTreeItem {
    const dbNode = new EncoreTreeItem(
      database.name,
      vscode.TreeItemCollapsibleState.None,
      "database",
    ...
    const hasConnUri = this.databaseStore.getConnUri(database.name) !== undefined;
...
function isPathInsideAppRoot(candidatePath: string, appRootPath: string): boolean {
...
function buildDiscoveredEncoreApp(encoreAppPath: string): DiscoveredEncoreApp {
...
function discoveredAppsEqual(
  left: readonly DiscoveredEncoreApp[],
  right: readonly DiscoveredEncoreApp[],
): boolean {
